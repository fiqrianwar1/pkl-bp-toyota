<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 flex items-center gap-2">
            {{-- 🔥 Ganti Icon ke Paint (misal: fa-paint-roller) --}}
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-2xl p-6 border border-gray-100">
                ...
                <div class="flex justify-between items-center mb-5">
                    {{-- Ganti Route ke paint.create --}}
                    <a href="{{ route('paint.create') }}" 
                        ...
                        <i class="fa-solid fa-plus mr-2"></i> Tambah Data Paint
                    </a>

                    {{-- Ganti Route ke paint.index --}}
                    <form method="GET" action="{{ route('paint.index') }}" class="flex gap-2 items-center">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full border border-gray-200 text-sm rounded-lg overflow-hidden">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            {{-- Ganti variabel $bodies -> $paints dan $body -> $paint --}}
                            @forelse ($paints as $paint)
                                <tr class="hover:bg-blue-50 transition">
                                    <td class="px-4 py-2">{{ $loop->iteration + $paints->firstItem() - 1 }}</td>
                                    <td class="px-4 py-2 font-semibold text-gray-800">{{ $paint->mekanik->nama ?? '-' }}</td>
                                    <td class="px-4 py-2">{{ $paint->spk->no_spk ?? '-' }}</td>
                                    <td class="px-4 py-2">{{ $paint->tanggal ? $paint->tanggal->translatedFormat('d F Y') : '-' }}</td>
                                    <td class="px-4 py-2">{{ $paint->jam_mulai ? \Carbon\Carbon::parse($paint->jam_mulai)->format('H:i') : '-' }}</td>
                                    <td class="px-4 py-2">{{ $paint->jam_selesai ? \Carbon\Carbon::parse($paint->jam_selesai)->format('H:i') : '-' }}</td>
                                    <td class="px-4 py-2">
                                        <span @class([
                                            'px-3 py-1 rounded-full text-xs font-semibold text-white',
                                            'bg-green-600' => $paint->status == 'Selesai',
                                            'bg-yellow-500' => $paint->status == 'Proses',
                                            'bg-red-500' => $paint->status == 'Antri',
                                            'bg-gray-400' => !in_array($paint->status, ['Selesai', 'Proses', 'Antri'])
                                        ])>
                                            {{ $paint->status ?? '-' }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-2 text-center">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <form action="{{ route('paint.destroy', $paint->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                            ...
                                            <a href="{{ route('paint.report', $paint->id) }}" target="_blank"
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-3 text-center text-gray-500 italic">
                                        <i class="fa-solid fa-circle-info mr-1"></i> Belum ada data Paint
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div id="detailModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 transition-opacity duration-300 ease-out opacity-0">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-xl p-6 relative transform scale-95 transition-transform duration-300 ease-out">
            <h2 class="text-lg font-semibold mb-4 border-b pb-2 text-gray-700 flex items-center gap-2">
                ...
                <i class="fa-solid fa-paint-roller text-blue-600"></i> Detail Pengerjaan Paint
            </h2>
    ...
    <script>
        // 🔥 GANTI URL ROUTE KE PAINT
        const baseReportUrl = "{{ route('paint.report', ['id' => 'PLACEHOLDER']) }}"; 
        const baseDetailUrl = "{{ route('paint.show', ['paint' => 'PLACEHOLDER']) }}"; // 🔥 Ganti ke route show
