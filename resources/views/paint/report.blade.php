...
<head>
    ...
    {{-- 🔥 Ganti Title ke Paint --}}
    <title>Laporan Paint - SPK {{ $paint->spk->no_spk ?? 'N/A' }}</title> 
    <style>
        body {
...
<body>
    ...
    <table class="info-table">
        <tr>
            ...
            <td width="300">: <strong>{{ $paint->spk->no_spk ?? '-' }}</strong></td>
            <td width="180" style="font-weight: bold;">Customer</td>
            <td>: {{ $paint->spk->customer->nama ?? '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Mekanik</td>
            <td>: {{ $paint->mekanik->nama ?? '-' }}</td>
            <td style="font-weight: bold;">Tanggal Pengerjaan</td>
            <td>: {{ $paint->tanggal ? \Carbon\Carbon::parse($paint->tanggal)->translatedFormat('d F Y') : '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Jam Mulai</td>
            <td>: {{ $paint->jam_mulai ? \Carbon\Carbon::parse($paint->jam_mulai)->format('H:i') : '-' }}</td>
            <td style="font-weight: bold;">Jam Selesai</td>
            <td>: {{ $paint->jam_selesai ? \Carbon\Carbon::parse($paint->jam_selesai)->format('H:i') : '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Status</td>
            <td>: <strong>{{ ucfirst($paint->status ?? '-') }}</strong></td>
            <td></td><td></td>
        </tr>
    </table>
    ...
    <table class="detail-table">
        ...
        <tbody>
            @php $bahan_tersimpan = is_array($paint->bahan) ? $paint->bahan : []; @endphp 
    ...
    <div class="signature">
        ...
        <div class="signature-box">
            ...
            <img src="{{ asset('images/ttdari.png') }}" class="ttd-img" alt="TTD" 
