...
<head>
    ...
    <title>Laporan Body - SPK {{ $body->spk->no_spk ?? 'N/A' }}</title> 
    <style>
        body {
...
<body>
    ...
    <table class="info-table">
        <tr>
            ...
            <td width="300">: <strong>{{ $body->spk->no_spk ?? '-' }}</strong></td>
            <td width="180" style="font-weight: bold;">Customer</td>
            <td>: {{ $body->spk->customer->nama ?? '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Mekanik</td>
            <td>: {{ $body->mekanik->nama ?? '-' }}</td>
            <td style="font-weight: bold;">Tanggal Pengerjaan</td>
            <td>: {{ $body->tanggal ? \Carbon\Carbon::parse($body->tanggal)->translatedFormat('d F Y') : '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Jam Mulai</td>
            <td>: {{ $body->jam_mulai ? \Carbon\Carbon::parse($body->jam_mulai)->format('H:i') : '-' }}</td>
            <td style="font-weight: bold;">Jam Selesai</td>
            <td>: {{ $body->jam_selesai ? \Carbon\Carbon::parse($body->jam_selesai)->format('H:i') : '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Status</td>
            <td>: <strong>{{ ucfirst($body->status ?? '-') }}</strong></td>
            <td></td><td></td>
        </tr>
    </table>
    ...
    <table class="detail-table">
        ...
        <tbody>
            @php $bahan_tersimpan = is_array($body->bahan) ? $body->bahan : []; @endphp 
    ...
    <div class="signature">
        ...
        <div class="signature-box">
            ...
            {{-- Mekanik TTD Manual (Kosongin gambar) --}}
            <br><br><br><br><br> 
            <u>{{ $body->mekanik->nama ?? '....................' }}</u>
        </div>

        {{-- TENGAH: FOREMAN --}}
        <div class="signature-box">
            <b>Foreman / Leader</b><br>
            {{-- Gambar TTD Foreman (Ganti nama filenya kalau ada, misal ttd_foreman.png) --}}
            <img src="{{ asset('images/ttdeko.png') }}" class="ttd-img" alt="TTD" 
                 onerror="this.style.display='none'; this.parentElement.innerHTML+='<br><br><br><br>';">
            
            {{-- Ambil nama Foreman (jika ada relasi) atau manual --}}
