...
<head>
    ...
    {{-- 🔥 Ganti Title ke Poles --}}
    <title>Laporan Poles - SPK {{ $poles_item->spk->no_spk ?? 'N/A' }}</title> 
    <style>
        body {
...
<body>
    ...
    <table class="info-table">
        <tr>
            ...
            <td width="300">: <strong>{{ $poles_item->spk->no_spk ?? '-' }}</strong></td>
            <td width="180" style="font-weight: bold;">Customer</td>
            <td>: {{ $poles_item->spk->customer->nama ?? '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Mekanik</td>
            <td>: {{ $poles_item->mekanik->nama ?? '-' }}</td>
            <td style="font-weight: bold;">Tanggal Pengerjaan</td>
            <td>: {{ $poles_item->tanggal ? \Carbon\Carbon::parse($poles_item->tanggal)->translatedFormat('d F Y') : '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Jam Mulai</td>
            <td>: {{ $poles_item->jam_mulai ? \Carbon\Carbon::parse($poles_item->jam_mulai)->format('H:i') : '-' }}</td>
            <td style="font-weight: bold;">Jam Selesai</td>
            <td>: {{ $poles_item->jam_selesai ? \Carbon\Carbon::parse($poles_item->jam_selesai)->format('H:i') : '-' }}</td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Status</td>
            <td>: <strong>{{ ucfirst($poles_item->status ?? '-') }}</strong></td>
            <td></td><td></td>
        </tr>
    </table>
    ...
    <table class="detail-table">
        ...
        <tbody>
            @php $bahan_tersimpan = is_array($poles_item->bahan) ? $poles_item->bahan : []; @endphp 
    ...
    <div class="signature">
        ...
        <div class="signature-box">
            ...
            <img src="{{ asset('images/ttdandik.png') }}" class="ttd-img" alt="TTD" 
